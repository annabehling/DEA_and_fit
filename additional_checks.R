...
# fit parental/hybrid DESeq2 results produced using independent filtering
...
# hybrid
hybrid_DEA_res <- hybrid_DE(results_dir = "./example_files", species = "HH_p", include_N = TRUE, parent_1 = "G_raimondii", parent_2 = "G_arboreum")
...
classes_df <- gene_cats(parent_classes, hybrid_classes, parent_1 = "G_raimondii", parent_2 = "G_arboreum", results_dir = "./example_files", species = "HH_p")


table(classes_df$classification) #resulting category count data
...
# reading in read count data
# hybrid
hybrid_DEA_res_2 <- hybrid_DE(results_dir = "./example_files", species = "HH_p", include_N = TRUE, parent_1 = "G_arboreum", parent_2 = "G_raimondii")

# parental
exp_file_HH_p_2 <- read_exp_file(results_dir = "./example_files", species = "HH_p")
parent_DEA_res_2 <- parental_DE(exp_file_HH_p_2, parent_1 = "G_arboreum", parent_2 = "G_raimondii")

# identify differentially expressed genes
# hybrid
hybrid_classes_2 <- fit_and_classify(hybrid_DEA_res_2, parent_1 = "G_arboreum", parent_2 = "G_raimondii", cutoff=1, min_p = 0.05)

# parental
parent_classes_2 <- fit_and_classify(parent_DEA_res_2, parent_1 = "G_arboreum", parent_2 = "G_raimondii", cutoff=1, min_p = 0.05)

classes_df_2 <- gene_cats(parent_classes_2, hybrid_classes_2, parent_1 = "G_arboreum", parent_2 = "G_raimondii", results_dir = "./example_files", species = "HH_p")

table(classes_df_2$classification) #resulting category count data == table(classes_df$classification)
#HEBi   HEBl    HER PEI de PEI eq 
#721    920     28    159   7714


## testing for extraneous effects of no independent filtering
...
parent_classes_IF <- fit_and_classify_IF(parent_DEA_res, parent_1 = "G_raimondii", parent_2 = "G_arboreum", cutoff=1, min_p = 0.05)
hybrid_classes_IF <- fit_and_classify_IF(hybrid_DEA_res, parent_1 = "G_raimondii", parent_2 = "G_arboreum", cutoff=1, min_p = 0.05)
...
all(parent_classes[, c(1:5)] == parent_classes_IF[, c(1:5)], na.rm = TRUE) #TRUE
all(hybrid_classes[, c(1:5)] == hybrid_classes_IF[, c(1:5)], na.rm = TRUE) #TRUE
